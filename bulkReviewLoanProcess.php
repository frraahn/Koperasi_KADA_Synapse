...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($loanSelections as $selection) {
        ...
        if ($action === 'approve') {
            $query = "UPDATE loan SET loanStatus = 5, loanReviewDate = NOW(), adminStaffNo = ? WHERE staffNo = ? AND loanID = ?";
        } elseif ($action === 'reject') {
            $query = "UPDATE loan SET loanStatus = 4, loanReviewDate = NOW(), adminStaffNo = ? WHERE staffNo = ? AND loanID = ?";
        } else {
            die("Invalid action.");
        }

        if ($stmt = mysqli_prepare($con, $query)) {
            mysqli_stmt_bind_param($stmt, "sii", $adminStaffNo, $staffNo, $loanID);
            ...
            if ($emailStmt = mysqli_prepare($con, $getEmailQuery)) {
                ...
                if ($emailRow = mysqli_fetch_assoc($emailResult)) {
                    ...
                    if (!empty($applicantEmail)) {
                        ...
                        if ($sendingEmail != 1) {
                            echo "<script>alert('$sendingEmail'); window.location.href = 'reviewLoanList.php';</script>";
                            exit;
                        }
                    } else {
    ...
    header("Location: reviewLoanList.php?status=success");
    exit();
} else {
