...
if ($result->num_rows > 0) {
    ...
    die("No family info found with the given Staff No.");
}
...
<div class="page-header">
  ...
  <div class="container">
        <div class="content-container">
            <h3>Maklumat Keluarga dan Pewaris</h3><br>
            <div class="table-container">
            <table class="table table-hover">
                <tr>
                    <th scope="row">Bilangan</th>
                    <th scope="row">Hubungan</th>
                    <th scope="row">Nama Ahli Keluarga</th>
                    <th scope="row">No. Kad Pengenalan @ Surat Beranak Ahli Keluarga</th>
                    <th scope="row">No Anggota</th>
                </tr>
            </thead>
            <tbody>
                <?php 
                $bilangan = 1; // Start the counter from 1
                foreach ($rows as $row): 
                ?>
                <tr>
                    <td><?php echo $bilangan++; ?></td> <!-- Increment the counter -->
                    <td><?php echo htmlspecialchars($row['relationship']); ?></td>
                    <td><?php echo htmlspecialchars($row['familyName']); ?></td>
                    <td><?php echo htmlspecialchars($row['familyIC']); ?></td>
                    <td><?php echo htmlspecialchars($row['staffNo']); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        <a href="adreviewMembership.php?staffNo=<?php echo urlencode($staffNo); ?>" class="btn btn-secondary mt-3">Kembali : Maklumat Pemohon</a>
        <a href="adreviewBankInfo.php?staffNo=<?php echo urlencode($staffNo); ?>" class="btn btn-primary mt-3">Seterusnya : Maklumat Bank</a>
    </div>
