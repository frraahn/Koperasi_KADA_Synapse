...
<head>
    ...
    <script>
        function confirmLengkap(staffNo) {
            const adminStaffNo = prompt("Masukkan nombor Kakitangan Admin:");
            if (adminStaffNo) {
                window.location.href = `reviewMembershipListProcess.php?staffNo=${encodeURIComponent(staffNo)}&action=lengkap&adminStaffNo=${encodeURIComponent(adminStaffNo)}`;
            }
        }
    </script>
<body>
    <div class="page-header">
    <h2 class="page-title">Permohonan Anggota</h2></div>
    ...
    <div class="container">
        ...
        <button class="btn btn-danger" onclick="bulkAction('reject')">Tolak Pilihan</button>
            ...
            <thead>
                <tr>
                    ...
                    <th scope="col">Tarikh Permohonan Disemak</th>
                </tr>
            </thead>
            <tbody id="tableBody">
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td><input type=\"checkbox\" class=\"staffCheckbox\" value=\"" . htmlspecialchars($row['staffNo']) . "\"></td>";
                    echo "<td>" . htmlspecialchars($row['staffNo'] ?? 'N/A') . "</td>";
                    echo "<td>
                            <a href='adreviewMembership.php?staffNo=" . urlencode($row['staffNo']) . "'>" 
                            . htmlspecialchars($row['applicantName'] ?? 'N/A') . 
                            "</a>
                          </td>";
                    echo "<td>" . htmlspecialchars($row['membershipApplyDate'] ?? 'N/A') . "</td>";
                    echo "<td>
                            <button onclick=\"confirmLengkap('" . htmlspecialchars($row['staffNo']) . "')\" class='btn btn-success btn-sm'>Lengkap</button>
                            <a href='reviewMembershipListProcess.php?staffNo=" . urlencode($row['staffNo']) . "&action=tidak lengkap' class='btn btn-danger btn-sm'>Tidak Lengkap</a>
                          </td>";
                    echo "<td>" . htmlspecialchars($row['statusDesc'] ?? 'N/A') . "</td>";
                    echo "<td>" . htmlspecialchars($row['membershipReviewDate'] ?? 'N/A') . "</td>";
                    echo "</tr>";
                }
     ...
     <script>
        ...
        function bulkAction(action) {
            ...
            let adminStaffNo = prompt("Masukkan nombor Kakitangan Admin:");
            if (!adminStaffNo) return;
            ...
            form.method = "POST";
            form.action = "bulkReviewProcess.php";
            ...
            inputAdmin.name = "adminStaffNo";
            inputAdmin.value = adminStaffNo;
